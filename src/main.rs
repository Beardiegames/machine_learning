...
impl Cell {
    fn new(pos: (usize, usize), neibs: [Option<usize>; 8], alive: bool) -> Self {
        Self { pos, neibs, alive, rotation: 2, heat: 0 }
    }
    ...
    fn update(&mut self, cells: &Vec<Cell>) {
        if let Some(neighbor) = self.looking_at() {
            self.alive = cells[neighbor].alive;
        } else {
            self.alive = false;
        }
    }

    fn looking_at(&self) -> Option<usize> {
        self.neibs[self.rotation]
    }
...
impl ConwayGrid {
    ...
    fn randomize(&mut self) {
        ...
        // debug get_neibs() methode
        // for n in &self.cells[0].neibs {
        //     println!("{}", match n { Some(val) => val.to_string(), _ => "_".to_string() });
        // }
    ...
    fn get_neibs(&self, x: usize, y: usize) -> [Option<usize>; 8] {
        let xmax = self.width-1;
        let ymax = self.height-1;
        [
            match y > 0 { true => Some(x + (y-1) * self.width), false => None },
            match x < xmax && y > 0 { true => Some(x+1 + (y-1) * self.width), false => None },
            match x < xmax { true => Some(x+1 + y * self.width), false => None },
            match x < xmax && y < ymax { true => Some(x+1 + (y+1) * self.width), false => None },
            match y < ymax { true => Some(x + (y+1) * self.width), false => None },
            match x > 0 && y < ymax { true => Some(x-1 + (y+1) * self.width), false => None },
            match x > 0 { true => Some(x-1 + y * self.width), false => None },
            match x > 0 && y > 0 { true => Some(x-1 + (y-1) * self.width), false => None },
        ]
    }

    fn update(&mut self) {
        self.scratch_cells = self.cells.clone();
        for y in 0..self.height {
            for x in 0..self.width {
                ...
                self.cells[idx].update(&self.scratch_cells);
            }
        }
    ...
    fn toggle(&mut self, x: isize, y: isize) -> bool {
        if let Some(i) = self.grid_idx(x, y) {
            ...
            self.cells[i].alive = !was_alive; //.set_alive(!was_alive);
            !was_alive
        } else {
            false
        }
    ...
    fn set_line(&mut self, x0: isize, y0: isize, x1: isize, y1: isize, alive: bool) {
        ...
        for (x, y) in line_drawing::Bresenham::new((x0, y0), (x1, y1)) {
            if let Some(i) = self.grid_idx(x, y) {
                self.cells[i].alive = alive; //set_alive(alive);
            } else {
                break;
            }
        }
